/**
 * @file game.js is the root file for this Racing game
 */
...
/**
 * @groundID returns ground model
 * @porscheID returns porsche model
 * @wheel1ID returns first wheel
 * @wheel2ID returns second wheel
 * @wheel3ID returns third wheel
 * @wheel4ID returns fourth wheel
 */
var groundID=''
var porscheID=''
var wheel1ID=''
...
function CullTrees(currpos,drawdist){
  scene.traverse( function( node ) {
    if ( node instanceof THREE.Group ) {
      ...
      if((distanceVector(node.position,currpos)>shadowDistance)  &&
      node.uuid!=groundID && node.uuid!=porscheID && 
      node.uuid!=wheel1ID && node.uuid!=wheel2ID &&
       node.uuid!=wheel3ID && node.uuid!=wheel4ID && node.uuid!=cubeID){
